	...
	//パートナーシナジーからサービスへ→キャプチャー
		...
		public void toService(WebDriver driver, WebDriverWait wait, String saveFolder)
			...
			if (windowWidth < (992 + 16)) {
				...
				//クリック対象要素が表示されるまで待つ→クリック（通常メニュー第一レベル）
				waitAndClick(driver, wait,
						"#menu-item-20 > a > span", "メニューサービス");

			}

			//サービスキャプチャー
			waitAndCapture(driver, wait, "body > div.page-contenet.content-out-box.w-max > div > h1 > span", "IEトップページからサービス",
					saveFolder);
			...
			//サービスキャプチャースクロール後
			waitAndCapture(driver, wait, "#privacymark-logo", "IEトップページからサービススクロール後", saveFolder);
